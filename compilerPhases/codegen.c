...
void generateCode(const char *inputFile, const char *outputAsmFile) {
    ...
    for (int i = 0; i < lineCount; i++) {
        ...
        if (strncmp(line, "return", 6) == 0) {
            ...
            while (*afterReturn == ' ' || *afterReturn == '\t') afterReturn++;
            if (*afterReturn != '\0') {
        ...
        if (sscanf(line, "%s = %s %s %s", lhs, op1, op, op2) == 4) {
            fprintf(asmFile, "    # %s\n", line);
            if (isConstant(op1)) fprintf(asmFile, "    movl $%s, %%eax\n", op1);
            else fprintf(asmFile, "    movl %s, %%eax\n", op1);

            if (strcmp(op, "+") == 0) {
                if (isConstant(op2)) fprintf(asmFile, "    addl $%s, %%eax\n", op2);
                else fprintf(asmFile, "    addl %s, %%eax\n", op2);
            } else if (strcmp(op, "-") == 0) {
                if (isConstant(op2)) fprintf(asmFile, "    subl $%s, %%eax\n", op2);
                else fprintf(asmFile, "    subl %s, %%eax\n", op2);
            } else if (strcmp(op, "*") == 0) {
                if (isConstant(op2)) fprintf(asmFile, "    imull $%s, %%eax\n", op2);
                else fprintf(asmFile, "    imull %s, %%eax\n", op2);
            } else if (strcmp(op, "/") == 0) {
                fprintf(asmFile, "    movl %%eax, %%ebx\n");
                if (isConstant(op2)) fprintf(asmFile, "    movl $%s, %%ecx\n", op2);
                else fprintf(asmFile, "    movl %s, %%ecx\n", op2);
        ...
        // Conditional jump
            ...
            if (strcmp(op, ">") == 0) fprintf(asmFile, "    jg %s\n", lhs);
            else if (strcmp(op, "<") == 0) fprintf(asmFile, "    jl %s\n", lhs);
            else if (strcmp(op, ">=") == 0) fprintf(asmFile, "    jge %s\n", lhs);
            else if (strcmp(op, "<=") == 0) fprintf(asmFile, "    jle %s\n", lhs);
            else if (strcmp(op, "==") == 0) fprintf(asmFile, "    je %s\n", lhs);
            else if (strcmp(op, "!=") == 0) fprintf(asmFile, "    jne %s\n", lhs);
        }
